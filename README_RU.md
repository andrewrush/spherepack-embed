...
![Сравнение границ](assets/spherepack_bounds_comparison.png)
...
Вывод: при росте размерности packing density падает
       ...
       Если Astra #1 будет независимо подтверждён, он может сузить
       теоретические границы для высоких n. Практическая польза
       для embeddings требует отдельных алгоритмов.
...
Измеренный бенчмарк на синтетических кластеризованных embeddings.
...
![Бенчмарк сжатия](assets/embed_compression_benchmark.png)
...
| Product Q (m=8) | 4× | ~0.275 | Сильно зависит от init |
...
### Практическая реальность и проклятие размерности

Векторные embeddings — это координаты данных в многомерном пространстве. Теоретические прорывы вроде Astra #1 доказывают, что более точные математические границы упаковки шаров *существуют* в высоких размерностях.

Однако между теоретическим существованием и конструктивными алгоритмами лежит фундаментальный разрыв. Наивная или жадная упаковка работает в низких размерностях (n ≤ 16), но страдает от **проклятия размерности** при росте n — легко наблюдаемо в `benchmark.py`. При n=768 (типично для BERT) плотность жадной упаковки падает практически до нуля.

**Этот проект — не production-оптимизатор RAG.** Это образовательный инструмент и бенчмарк, созданный для:

1. Визуализации разрыва между теоретическими математическими границами и практическими алгоритмическими ограничениями.
2. Лёгкого, воспроизводимого тестового стенда (оптимизированного для Termux/aarch64) для экспериментов с метриками упаковки.
3. Демонстрации того, как новые математические доказательства могут в будущем повлиять на стратегии квантизации и индексации, даже если текущие жадные методы далеки от предела.

**Реалистичная проверка:** знание о существовании более точного предела не уменьшает векторную базу автоматически. Всё ещё нужны реальные алгоритмы (квантизация, ANN-индексы, снижение размерности). Этот проект показывает и теоретический потолок, и честные измерения практических методов.
